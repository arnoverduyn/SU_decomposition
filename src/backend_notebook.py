...
def plot_trajectory(T,path_to_data,path_to_figures):
    (kettle,_,_,_) = src.data_handling.retrieve_data_designed_objects(path_to_data)
    src.plotting.plot_trajectories_with_kettle(T, [T], kettle, path_to_figures)

def plot_U(U,dt,path_to_figures):
    N = U[0].shape[2]
    time_total = (N-1)*dt
